...
const Player = {
    ...
    setupSidebarToggle: () => {
        ...
        if (!layout || !btnCollapse || !btnRestore)
            return;
    ...
    setupAuthUI: (user) => {
        ...
        if (profileRole)
            profileRole.textContent = user.role === 'INSTRUCTOR' ? 'Instrutor' : 'Estudante';
    ...
    renderSidebar: () => {
        ...
        Player.courseData.modules.forEach((mod, index) => {
            ...
            if (isOpen)
                Player.openModules.add(mod.id); // Ensure initial is tracked
    ...
    showCertificateButton: () => {
        ...
        if (document.getElementById('btn-generate-certificate'))
            return;
