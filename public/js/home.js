...
export const Home = {
    ...
    currentPage: 1,
    itemsPerPage: 12,
    cartItems: [], // Track simulated cart items
    /**
     ...
     */
    openCourseModal: async (courseId) => {
        try {
            ...
            if (modulesList)
                modulesList.innerHTML = '<p class="loading-msg">Carregando módulos...</p>';
            ...
            const img = document.getElementById('modal-course-img');
            ...
            if (instructor)
                instructor.textContent = course.instructor?.name || 'Instrutor Desconhecido';
            // Format Date
            if (date) {
